...
impl DeviceBuffer {
    pub fn new(context: &hotham::contexts::VulkanContext) -> DeviceBuffer {
        match context.physical_device_properties.device_type {
            ...
            invalid => unreachable!("What? {invalid:?}"),
        }
    }
...
impl DiscreteDeviceBuffer {
    pub fn new(context: &hotham::contexts::VulkanContext) -> DiscreteDeviceBuffer {
        todo!();
...
fn create_slab_buffer(context: &hotham::contexts::VulkanContext) -> vk::Buffer {
    let device = &context.device;
...
impl IntegratedDeviceBuffer {
    pub fn new(context: &hotham::contexts::VulkanContext) -> IntegratedDeviceBuffer {
        let device = &context.device;
        let memory_properties = &context.memory_properties;

        let slab_buffer = create_slab_buffer(&context);

        let mut memory_requirements = vk::MemoryRequirements2::default();
        unsafe {
            device.get_buffer_memory_requirements2(
                &vk::BufferMemoryRequirementsInfo2::default().buffer(slab_buffer),
                &mut memory_requirements,
            )
        };

        // Search through the available memory types to find the one we want
        let (memory_type_index, memory_heap_index) =
            find_memory(memory_properties, memory_requirements.memory_requirements);
        ...
        // Bind it!
        unsafe { device.bind_buffer_memory(slab_buffer, global_memory, 0) }.unwrap();

        // Now rew.. I mean, get its address:
        let slab_address = unsafe {
            device.get_buffer_device_address(
                &vk::BufferDeviceAddressInfo::default().buffer(slab_buffer),
            )
        };
