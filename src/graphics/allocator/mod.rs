...
pub const GLOBAL_MEMORY_SIZE: u64 = 1073741824;
...
impl Allocator {
    pub fn new(context: &hotham::contexts::VulkanContext) -> Self {
        let backend = DeviceBuffer::new(context);
    ...
    pub fn allocate_buffer<T: Sized>(
        ...
        self.allocate_buffer_inner(align, handle, size, context)
    }
    ...
    pub fn execute_transfers(
        ...
        // context.begin_marker("Execute Transfers", glam::vec4(0., 0., 1., 1.));
...
pub fn find_memory(
    ...
    let type_filter = memory_requirements.memory_type_bits;
    for (index, memory_type) in memory_properties.memory_types_as_slice().iter().enumerate() {
        // We need visible and coherent memory
        if !memory_type.property_flags.contains(
            vk::MemoryPropertyFlags::HOST_VISIBLE | vk::MemoryPropertyFlags::HOST_COHERENT,
        ) {
            continue;
        ...
        // Whatever the buffer requires, so do we
