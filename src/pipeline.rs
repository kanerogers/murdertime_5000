...
pub fn load_module(data: &[u8], context: &hotham::contexts::VulkanContext) -> vk::ShaderModule {
    let mut data = std::io::Cursor::new(data);
    let words = hotham::ash::util::read_spv(&mut data).unwrap();
    log::error!("Shader is {} words", words.len());
